...
//						
//			// SqlSession 객체변수를 이용하여 처리할 SQL문을 호출해서 실행한다.
//			// select문의 처리 결과가 여러 개일 경우에는 selectList()메서드를 사용하는데 
//			// 이 메서드느 여러 개의 레코드 각각을 VO객체에 담은 후 이 VO객체를 List에 추가해 주는
//			// 작업을 자동으로 수행한다.
//			
//			// 형식) session.selectList("namespace속성값.id속성값", 파라미터클래스)
//		    //			반환값 ==> resultType에 지정한 객체가 저장된 List객체
//			
//			List<LprodVO> lprodList = session.selectList("lprod.getAllLprod",gu);
//			
