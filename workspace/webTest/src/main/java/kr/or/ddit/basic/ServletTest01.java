...
// 이 예제는 배포 서술자(web.xml)을 이용해서 실행할 Servlet을 설정하여 처리하는 예제이다.
public class ServletTest01 extends HttpServlet {
	// 이 곳에서는 대부분 service()메서드 또는 doGet()메서드나 doPost()메서드를 재정의해서 작성한다.
	
	/* 
		전체 주소: http://localhost:8000/webTest/servletTest01.do
		- http: 프로토콜
		- localhost: 컴퓨터 이름(도메인명) 또는 IP주소
		- 8000: PORT번호(포트번호가 80번일 경우에는 생략가능)
	...
	*/
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		response.setCharacterEncoding("UTF-8"); // 응답 문서의 인코딩 방식 설정하기
