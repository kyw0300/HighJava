...
public class CookieAdd extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 형식) Cookie cookie 변수 = new Cookie("쿠키이름", "쿠키값");
		//		 쿠키값으로 한글을 사용할 경우에는 URLEncoder.encode() 메서드로 인코딩한 후 저장한다.
		Cookie nameCookie = new Cookie("name", URLEncoder.encode("오도화","utf-8"));
		Cookie ageCookie = new Cookie("age", "42");
		...
		*/
		...
		out.println("<a href='" + request.getContextPath() + "/basic/cookie/cookieTest01.jsp'>"
				+ "시작문서로 이동하기</a>");
