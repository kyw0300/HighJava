...
/*
	...
	2. Servlet Container는 web.xml에 정의된 URL패턴을 확인하여 어느 서블릿을 통해 처리해야 할지를 검색한다.
	   (해당 서블릿이 로딩이 안 된 경우에는 로딩하고 처음 로딩시 init()메서드를 호출한다.)
	   (Servlet버전 3.0이상에서는 어노테이션(@WebServlet)으로 설정할 수 있다.)
...
//		super.service(request, response); // get방식이냐 post방식이냐 구분
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out = response.getWriter();
		out.println("<html><head><meta charset='utf-8'></head>"
				+ "<body><h1 style='color:green;'>doPost()메서드 처리 내용</h1></body></html>");
