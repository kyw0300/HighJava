	...
	/*
		- redirect
			==> 다른 페이지로 넘어가도록 하는 기능을 말한다.
				...
				이동하는 방식을 말한다. (재 요청할 때는 GET방식으로의 요청이기 때문에 URL의 길이에 제한이 있다.)
...
//		request.setAttribute("tel",	"010-9999-9999");	// redirect에서는 이런식으로 데이터를 보낼 수 없다.
		...
		// 이 문서에서 만들어진 데이터를 Redirect문서로 보내려면 Get방식의 파라미터로 재구성해서 보내야 한다.
		...
		name = URLEncoder.encode(name,"UTF-8"); // 이거 안해주면 한글이 코드화가 되지 않는다.
		
		response.sendRedirect(request.getContextPath()+
						"/redirectTest.do?username2=" + name + "&tel=" + tel);
	}
