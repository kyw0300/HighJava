...
public class JdbcTest01 {
	...
	1. 드라이버 로딩 ==> 라이브러리를 사용할 수 있게 메모리로 읽어 들이는 작업
		==> JDBC API버전 4이상에서는 getConnection() 메서드에서 자동으로 로딩해주기 때문에 생략 가능
	...
	4. 결과 처리 ==> 질의 결과를 받아서 원하는 작업을 수행한다.
		1) SQL문 select문일 경우에는 select한 결과가 ResultSet 객체에 저장되어 반환된다.
	...
	*/
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 3-2. Statement 객체 또는 PreparedStatement 객체 생성
			...
			while(rs.next()) {
				// ResultSet 객체의 데이터를 가리키는 포인터가 가리키는 곳의 데이터를 가져오는 방법
