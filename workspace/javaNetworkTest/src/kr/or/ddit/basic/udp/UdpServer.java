...
//				String msg = new String(inpacket.getData(), 0, inpacket.getLength(), "UTF-8"); // 방법2
				...
				if("/end".equals(msg)) { // 수신 문자열이 '/end'면 작업 끝... 
					break;
				}
				...
				// 송신용 패킷 객체 생성
				// 	==> 전송할 데이터가 저장된 byte형 배열, 전송할 데이터의 길이(배열 길이),
				//		상대방의 주소정보(InetAddress객체), 포트번호 이 4가지를 생성자의 인수값으로 지정하여 생성
