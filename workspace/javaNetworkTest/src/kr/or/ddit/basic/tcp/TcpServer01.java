...
public class TcpServer01 {
	public static void main(String[] args) throws IOException {
		...
		//   ==> 클라이언트의 연결 요청이 오면 새로운 Socket개체를 생성해서 클라이언트의 Socket객체와 연결한다.
		Socket socket = server.accept();
		
		// 이 accept() 메서드 명령을 기술한 이후의 내용은 클라이언트와 연결된 후에 처리할 내용들을 작성하면 된다.
		System.out.println("클라리언트와 연결되었습니다...");
		System.out.println();
		...
		System.out.println("현재 컴퓨터의 IP 주소 : " + socket.getLocalAddress());
		System.out.println("현재 컴퓨터의 Port 번호 : " + socket.getLocalPort());
		System.out.println();
		System.out.println("----------------------------------------------------");
