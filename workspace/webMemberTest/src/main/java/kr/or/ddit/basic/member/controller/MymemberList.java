package kr.or.ddit.basic.member.controller;

import java.io.IOException;
import java.util.List;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import kr.or.ddit.basic.member.service.MymemberService;
import kr.or.ddit.basic.member.vo.MymemberVO;

@WebServlet("/mymemberList.do")
public class MymemberList extends HttpServlet {
	private static final long serialVersionUID = 1L;

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.setCharacterEncoding("utf-8");
		response.setCharacterEncoding("utf-8");
		
		MymemberService service = MymemberService.getInstance();
		List<MymemberVO> list = service.selectAllMember();
		
		request.setAttribute("mymemberlist", list);
		request.getRequestDispatcher("/memberView/memberView.jsp").forward(request, response);
	}

	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		doGet(request, response);
	}

}
