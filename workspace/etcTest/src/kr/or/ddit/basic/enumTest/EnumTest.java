...
/*
	enum(열거형)  ==>  서로 관련있는 상수들의 집합을 나타낸다.
	 ...
	 - 열거형의 속성 및 메서드
	  ...
	  열거형이름.valueOf("열거형상수명")  ==>  지정된 열거형에서 '열거형상수명'과 
	  									일치하는 열거형 상수를 반환한다.
...
public class EnumTest {
	...
	public enum Season {
		...
		// 생성자
		private Season(String span, int temp) {		// 묵시적으로 private형태
			this.span = span;
			this.temp = temp;
		}
...
//		}
		...
		// switch의 case문에 열거형을 배치할 때는 '열거형이름'을 생략하고 상수명만 기술해야 한다.
		switch (mycnt) {
		...
		for(Season s : Season.values()) {
			System.out.println(s.name() + " == " + s + " ==> " + s.getSpan() + "의 평균온도 : " + s.getTemp());
		}
