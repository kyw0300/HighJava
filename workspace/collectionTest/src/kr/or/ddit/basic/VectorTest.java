...
public class VectorTest {
	public static void main(String[] args) {
		...
		v1.add("aaaa");
		v1.add(new Integer(111)); // wrapper class, 예전 방법
		v1.add(123); // 요즘은 오토박싱, 오토언박싱이 지원
		v1.add('a'); // 'char'형
		v1.add(true);
		boolean r = v1.add(3.14); // 값들은 모두 Object형식으로 저장됨

		System.out.println("현재 크기: " + v1.size());
		System.out.println("반환값 :" + r);

		// 데이터 추가하기2 : addElement(추가할 데이터),add와 거의 유사
		// ==> 이전 버전의 프로그램과의 호환성을 위해서 남아 있는 메서드
		v1.addElement("CCC");

		// System.out.println("v1 => " + v1.toString());
		System.out.println("v1 => " + v1); // toString 생략 가능
		...
		int iTemp = (int) v1.get(2); // (Integar)로도 가능, 강제 타입변환, 변수에 저장할 때는 변환해서 넣어줘야함
		...
		// ==> 반환값 : true(삭제성공), false(삭제실패)
		// ==> 삭제할 데이터가 '정수형'이거나 'char'형 일 경우에는 반드시 객체로 변환해서 사용해야 한다.
		v1.remove("CCC");
		System.out.println("CCC 삭제 후 v1 => " + v1);

		// v1.remove(123); => 배열의 123번째를 지우라는말, 오류남
		// v1.remove(new Integer(123)); // 방법1 : 객체화 (1.9버전부터는 사용불가)
		v1.remove(Integer.valueOf(123)); // 방법2 추천!
		System.out.println("123 삭제 후 v1 => " + v1);

		// v1.remove('a'); // 아스키 코드값 97로 인식함
		...
		/* 
		 제네릭타입(Generic Type) ==> 클래스 내부에서 사용할 데이터 타입을 객체를 생성할 때
			...
			- 이때 제네릭으로 선언 될 수 있는 데이터 타입은 클래스형 이어야 한다.
			  => int는 Integar, boolean은 Boolean, char는 Character 등으로 대체해서 사용해야 함.
		...
		*/
		...
		Vector<Vector> vv = new Vector<>(); // ≒ 이차원 배열
