...
public class SetTest {
	public static void main(String[] args) {
		...
		// Set의 데이터 삭제하기 ==> remove(삭제할 자료) ==> 반환값 : true(삭제성공), false(삭제실패)
		// ==> clear() : 전체 데이터 삭제
...
//		System.out.println("Set 데이터 hs1 => " + hs1);
		...
		/*
		 * Set의 데이터 순서(index)가 없기 때문에 List처럼 index로 데이터를 하나씩 불러올 수 있다. 그래서 데이터를 하나씩 차례로
		 * 얻기 위해서는 Iterator형 객체로 변환해야 한다. - Set형의 데이터들을 Iterator형 객체로 변환하는 메서드 ==>
		 * iterator()
		 */
		...
		/*
		 * Iterator의 hasNext()메서드 ==> Iterator의 포인터가 가리키는 곳의 다음 번째에 데이터가 있는지 검사한다. 데이터가
		 * 있으면 true, 없으면 false를 반환
		 */
		while (it.hasNext()) {
			/*
			 * Iterator의 next()메서드 ==> Iterator의 포인터를 다음 번째 위치로 이동시킨 후 그 곳의 데이터를 꺼내서 반환한다.
			 */
